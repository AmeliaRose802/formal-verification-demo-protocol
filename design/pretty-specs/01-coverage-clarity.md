...
| **R3 stateful** | Mutates member state rather than mapping input→output. | These functions need a *stateful pre/post spec* over the heap rather than a pure value-equality spec. saw-spec-gen supports that shape now; the remaining blocker in this repo is the concrete `KeyStore` surface (`std::optional<EnrollmentKey>`, lock plumbing, and override details), not a missing general capability. See [03-stateful-method-specs](03-stateful-method-specs.md). | `KeyStore::provision`, `KeyStore::activate` |
